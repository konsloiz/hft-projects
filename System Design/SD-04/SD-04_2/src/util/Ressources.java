...
public class Ressources {
	...
	private static List<String> getClassNames(File dir, int prefixLength, List<String> classNames,
			boolean includingSubpackages) {
		Stream.of(dir.listFiles((file, name) -> name.endsWith(CLASS_EXT) && !name.contains("$")))
				...
				.map(name -> name.replace(File.separatorChar, PACKAGE_SAPARATOR)).forEach(name -> classNames.add(name));
